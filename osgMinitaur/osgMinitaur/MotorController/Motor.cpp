...
float BlCon34::getRawPosition() {
  ...
  return (DARTMotorPos[inPin]] % TWO_PI);
}

void BlCon34::sendOpenLoop(float val) {
  //analogWriteFloat(outPin, (enableFlag) ? map(val, -1, 1, 0.12, 0.88) : 0);
  
  // outPin needs to be set as [0,1,2,3,4,5,6,7]
  // torqueCommand needs to be declared somewhere as a global
