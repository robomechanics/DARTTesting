	...
	if (STATE[gamepad].flags & FLAG_CONNECTED) {
		...
		while (read(STATE[gamepad].fd, &je, sizeof(je)) > 0) {
			...
			case JS_EVENT_BUTTON:
				...
				switch (je.number) {
				case 0: button = BUTTON_X; break;
				case 1: button = BUTTON_A; break;
				case 2: button = BUTTON_B; break;
				...
				case 10: button = BUTTON_LEFT_THUMB; break;
			...
			case JS_EVENT_AXIS:
				...
				case 2:	STATE[gamepad].stick[STICK_RIGHT].x = je.value; break;
				//case 3:	STATE[gamepad].stick[STICK_RIGHT].x = je.value; break;
				case 6:	STATE[gamepad].stick[STICK_RIGHT].y = je.value; break;
				case 3:	STATE[gamepad].stick[STICK_RIGHT].y = -je.value; break;
				case 7:	STATE[gamepad].trigger[TRIGGER_RIGHT].value = (je.value + 32768) >> 8; break;
				case 4:
					if (je.value == -32767) {
...
static void GamepadUpdateStick(GAMEPAD_AXIS* axis, float deadzone) {
	...
	if (axis->length > deadzone) {
		...
		axis->nx = axis->x / 32767.0f;//; / axis->length;
		axis->ny = axis->y / 32767.0f;//; / axis->length;
